...
func printSuggestion(code string) {
	...
	case "ERR_INVALID_APIKEY":
		...
		fmt.Fprintln(os.Stderr, "   → 运行 minixia config show 查看当前配置")
		fmt.Fprintln(os.Stderr, "   → 运行 minixia init 重新配置")
	case "ERR_QUOTA_EXCEEDED":
		fmt.Fprintln(os.Stderr, "   当日配额已用尽，请升级套餐或等待次日重置：")
		fmt.Fprintln(os.Stderr, "   → https://miniclaw.app/upgrade")
	case "ERR_PLAN_LIMIT":
	...
	fmt.Fprintln(os.Stderr, "   → 文档：https://docs.minixia.app/cli/troubleshooting")
}
