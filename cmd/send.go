...
import (
	...
	"github.com/jinwoll/minixia-cli/internal/media"
...
func init() {
	sendCmd.Flags().StringVarP(&sendType, "type", "t", "text", "消息类型：text / image / voice / voice_call")
...
func processContentByType(msgType, content string) (string, error) {
	...
	case "voice", "voice_call":
		// 音频文件路径 → base64 编码；纯文本 → 直接发送（服务端做 TTS）
		if media.FileExists(content) && media.IsAudioFile(content) {
			encoded, err := media.EncodeFileBase64(content)
			if err != nil {
				return "", err
			}
		...
		// 纯文本直接发送，服务端会调用 Azure TTS 合成
		return content, nil

	default:
		return "", fmt.Errorf("不支持的消息类型: %s（可用：text, image, voice, voice_call）", msgType)
	}
}
