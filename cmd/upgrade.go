...
var upgradeCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if upgradeCheckOnly {
			fmt.Printf("运行 minixia upgrade 执行升级。\n")
			return nil
		}
...
func fetchLatestVersion() (string, string, error) {
	...
	req.Header.Set("User-Agent", "minixia-cli")
	...
	filename := fmt.Sprintf("minixia-%s-%s%s", osName, arch, ext)
