...
var configListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(names) == 0 {
			output.PrintInfo("暂无 profile，运行 minixia init 创建。")
			return nil
		}
...
var configUseCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !config.ProfileExists(name) {
			return fmt.Errorf("profile \"%s\" 不存在，运行 minixia config create %s 创建", name, name)
		}
...
var configDeleteCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if global != nil && global.CurrentProfile == name {
			return fmt.Errorf("不能删除当前正在使用的 profile \"%s\"，请先 minixia config use <other>", name)
		}
