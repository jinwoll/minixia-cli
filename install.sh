#!/bin/sh
# 迷你虾 CLI 一键安装脚本（Linux / macOS / WSL / Git Bash）
# 用法: curl -fsSL https://raw.githubusercontent.com/jinwoll/minixia-cli/main/install.sh | sh
set -e

# ===== 配置 =====
# GitHub Releases：/releases/latest/download/ 下需包含各平台二进制与 checksums.sha256（由 GoReleaser 生成）
RELEASES_BASE="${MINIXIA_BINARY_URL:-https://github.com/jinwoll/minixia-cli/releases/latest/download}"
...
main() {
    ...
    echo ""
    success "🚀 下一步：运行 minixia init 开始配置"
}
