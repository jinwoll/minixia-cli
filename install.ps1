# 迷你虾 CLI 一键安装脚本（Windows PowerShell）
# 用法: irm https://raw.githubusercontent.com/jinwoll/minixia-cli/main/install.ps1 | iex
$ErrorActionPreference = "Stop"

# ===== 配置 =====
$ReleasesBase = if ($env:MINIXIA_BINARY_URL) { $env:MINIXIA_BINARY_URL } else { "https://github.com/jinwoll/minixia-cli/releases/latest/download" }
$InstallDir = if ($env:MINIXIA_INSTALL_DIR) { $env:MINIXIA_INSTALL_DIR } else { "$env:LOCALAPPDATA\minixia" }
$BinaryName = "minixia.exe"
...
# ===== 构造下载 URL =====
$FileName = "minixia-windows-${Arch}.exe"
$DownloadUrl = "$ReleasesBase/$FileName"
$ChecksumUrl = "$ReleasesBase/checksums.sha256"

# ===== 下载到临时目录 =====
$TmpDir = Join-Path $env:TEMP "minixia-install-$(Get-Random)"
...
Write-Host ""
Write-Success "🚀 下一步：运行 minixia init 开始配置"
